...
data class EventData(
    ...
    fun getFormattedPublished(): String {
        return published.let { textData: String ->
            ...
            dateTime.format(DateTimeFormatter.ofPattern(formattedDataAndTime))
        }
    }

    /**
     *  Возвращает отформатированную строку даты и времени проведения события в формате "yyyy-MM-dd HH:mm:ss".
     *
     * @return String Отформатированная строка даты и времени.
     */
    fun getFormattedDataAndTimeEvent(): String {
        return published.let { textData: String ->
            val dateTime = LocalDateTime.parse(textData, DateTimeFormatter.ISO_LOCAL_DATE_TIME)
            dateTime.format(DateTimeFormatter.ofPattern(formattedDataAndTime))
        }
